...
def get_targets_by_ms(path_target, time_ms):

    # Load up values of ms to compare to
    target_counts = np.loadtxt(path_target, dtype=int, delimiter=' ')
    ...
    print("Shape of target values: ", target_shape)
    ...
    while p < target_shape[0]:
        ...
        min_ms = target_counts[p][0]
        max_ms = target_counts[p][1]

        while (not target_added) and (ms_index < samples):
            ms_diff = time_ms[ms_index + 1] - time_ms[ms_index]

            # Check if the next time is from a new second. Adjust the difference if it is.
            if ms_diff < 0:
                ms_diff += 1000


            # Add the time difference to the ongoing sum
            ms_sum += ms_diff
            ms_index += 1

            # Check if the maximum time stamp has been passed for this target.
            # If it isn't, add the target value to this index
            if ms_sum > max_ms:
                break

            elif ms_sum > min_ms:
                target_output[ms_index] = target
